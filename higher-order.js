...
let totalPopulation = populations.reduce((runningTotal, curElement, curIndex, wholeArray) => {
  return runningTotal + wholeArray},0)
  return result
...
let orderTotals = {
  function calculatePrice(tax){
    return `${this.name} costs` + (this.price +(this.price * (tax/100))).toFixed(2)
  }
}
...
function someFunction (element,index, array){}
